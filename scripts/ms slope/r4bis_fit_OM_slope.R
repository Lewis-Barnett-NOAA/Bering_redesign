  ...
  for (iregion in c("bs_slope", "bs_shelf")[2]) {
    ...
    data_geostat <-
      readRDS(file = paste0("data/data_processed/species/", 
                            species_list$SCIENTIFIC_NAME[ispp], 
                            "/data_geostat.rds")) |>
    ...
    names(x = data_geostat) <- c("Lat", "Lon", "Year", "Species", "Weight_kg",
                                 "Effort", "Depth", "Temp", "Region", "CPUEkgkm")
    
    ## Filter interpolation grid to region
    interpolation_grid <- 
      subset(x = grid_bs, 
             ...
             & Year %in% list(
               "bs_slope" = 2002:2016,
               "bs_shelf" = seq(from = ifelse(test = species_list$SCIENTIFIC_NAME[ispp]
    ...
    n_interpolation_cells <- table(interpolation_grid$Year) |> unique()
    ...
    data_geostat1 <- rbind(
      ...
      data.frame(Lat = interpolation_grid$Lat,
                 ...
                 Effort = interpolation_grid$Area_in_survey_km2,
    ...
    ## Anisotropy
    aniso <- ifelse(
      test = species_list$SCIENTIFIC_NAME[ispp] %in% c("Anoplopoma fimbria",
                                                       "Atheresthes stomias") 
      & iregion == "bs_slope",
      yes = FALSE, 
      no = TRUE
    ...
    ## Newton Steps
    steps <- ifelse(
      test = species_list$SCIENTIFIC_NAME[ispp] %in% c("Hippoglossoides elassodon",
    ...
    if (species_list$SCIENTIFIC_NAME[ispp] %in% 
