...
for (species_name in species_list$SCIENTIFIC_NAME[c(6,9)]) { ## Loop over species -- start
  
  output_dir <- paste0("output/om/bs_shelf/", species_name, "/")
  ...
  new_folder <- drive_mkdir(
    name = species_name,
    path = "https://drive.google.com/drive/folders/15b3B0ZLros2HW-ctc1BjAVHX1in4yy4J"
  )
  
  ## Filter cpue to species and region, create a factor for year
  cpue_data <- subset(x = cpue_bs_allspp, 
                      subset = scientific_name == species_name & f_region == "bs_shelf") 
  ...
  for (model_type in c("cp_base", "cp_sbt", "cp_d_sbt")) { ## Loop over model types -- start
    
    ## Specify model formula for covariates (all have cold pool as a spatially 
    ## varying coefficient)
    model_formula <- list(
      "cp_base" = formula(x = cpue_kgkm2 ~ 0 + f_year),
      "cp_d_sbt" = formula(x = cpue_kgkm2 ~ 0 + f_year + poly(scaled_depth, degree = 2) + poly(scaled_roms_sbt, degree = 2)),
      # "cp_d" = formula(x = cpue_kgkm2 ~ 0 + f_year + poly(scaled_depth, degree = 2),
      "cp_sbt" = formula(x = cpue_kgkm2 ~ 0 + f_year +  poly(scaled_roms_sbt, degree = 2))
    )[[model_type]]
    ...
    jpeg(filename = paste0("output/om/bs_shelf/", species_name, 
    ...
    for (ifile in c(paste0(c("fit_", "sim_res_", "dharma_resids_"), 
      ...
      googledrive::drive_upload(
        ...
        path = paste0("Bering_redesign/om/bs_shelf/", species_name, "/"),
        name = ifile
      )
    }
