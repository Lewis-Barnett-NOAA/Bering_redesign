...
####################################################################
##
##    Gear calibration for the BS slope hauls
...
data_sratio <- readRDS(file = 'data/data_raw/shelf_slope_sratio_bootstrap.rds')

#convert SR of one species into another
data_sratio[which(data_sratio$SPECIES_CODE == '471'), 'SPECIES_CODE'] <- '472'

#read length raw data
data_length <- readRDS(file = 'data/data_raw/ak_bts_ebs_nbs_slope.rds')
head(data_length)
...
cruisejoin_ebs <- subset(x = data_length$cruise, 
                         subset = SURVEY == 'EBS')$CRUISEJOIN

#get hauls for the ebs
hauls_ebs <- subset(x = data_length$haul, 
                    subset = CRUISEJOIN %in% cruisejoin_ebs)

#convert time
time_axis <- as.POSIXct(x = hauls_ebs$START_TIME, 
                        origin = "1900-01-01", 
                        tz = "GMT") 
hauls_ebs$YEAR <- format(x = time_axis, format = "%Y")

#check hauls number per year in ebs
aggregate(HAULJOIN ~ YEAR, data = hauls_ebs, FUN = length)

#code species
spp_code <- unique(x = data_length$species[, c('SPECIES_CODE',
                                               "REPORT_NAME_SCIENTIFIC")])
names(x = spp_code) <- c('species_code', "scientific_name")
spp_code1 <- spp_code[which(spp_code$scientific_name %in% spp),]

#add Alaska skate
spp_code1 <- rbind(spp_code1, c(472,'Bathyraja aleutica'))

#merge sr data to species
data_sratio <- merge(x = data_sratio, y = spp_code1,
                     by.x = 'SPECIES_CODE', by.y = 'species_code')

#remove outlier
data_sratio <- subset(x = data_sratio, subset = s12<100)
...
names(data_sratio_summary) <- 
  c("SIZE_BIN", "scientific_name", "mean", "p90", "p10")
...
#plot
ggplot()+
  geom_point(data = data_sratio_summary,
             aes(x = SIZE_BIN, y = mean, group = SIZE_BIN)) +
  geom_point(data = data_sratio_summary, 
             aes(x = SIZE_BIN, y = p90, group = SIZE_BIN), color = 'red') +
  geom_point(data = data_sratio_summary, 
             aes(x = SIZE_BIN, y = p10, group = SIZE_BIN), color = 'blue') +
...
#save plot
ragg::agg_png(paste0('figures/slope/sr2.png'), 
...
lwgoa <- read.csv(file = 'data/data_raw/LenWtParams_GOA.csv')
lwai <- read.csv(file = 'data/data_raw/LenWtParams_AI.csv')
...
sp_data <- unique(x = coef_wl[complete.cases(coef_wl), 'spp'])
sp_rem <- setdiff(x = spp_code1$scientific_name, y = sp_data)

#get cruisejoin for the slope
head(data_length$cruise)
cruisejoin_bss <- subset(x = data_length$cruise,
                         subset = SURVEY == 'BSS')$CRUISEJOIN

#get hauls for the slope
hauls_bss <- subset(x = data_length$haul,
                    subset = CRUISEJOIN %in% cruisejoin_bss)

#convert time
time_axis <- as.POSIXct(x = hauls_bss$START_TIME, 
                        origin = "1900-01-01", 
                        tz = "GMT") 
hauls_bss$YEAR <- format(x = time_axis, format = "%Y")

#length data slope
length_bss <- data_length$size[
  which(data_length$size$HAULJOIN %in% unique(x = hauls_bss$HAULJOIN)),
]
length_bss <- merge(x = length_bss,
                    y = hauls_bss[, c('CRUISEJOIN', 'HAULJOIN', 'YEAR')],
                    by = c('CRUISEJOIN', 'HAULJOIN'))
length_bss <- merge(x = length_bss, y = spp_code1,
                    by.x = 'SPECIES_CODE', by.y = 'species_code')
...
#data s12
data_sratio2 <- data_sratio_summary[, c('mean','p90','p10',
                                        'scientific_name','LENGTH')]

#loop over combinations
for (r in 1:nrow(x = length_bss)) {
  ...
  #sex
  sex <- as.character(x = length_bss[r, 'SEX'])
  sex <- ifelse(test = sex == '3', yes = 'all', no = sex)
  ...
  if (all(is.na(x = coef_wl1$log_a))) {
    ...
    coef_wl1 <- data.frame('spp' = sp, 'sex' = sex, 'year' = NA, 
                           'log_a' = mean(x = coef_wl1$log_a, na.rm = TRUE),
                           'b' = mean(x = coef_wl1$b, na.rm = TRUE))
  }
  
  #convert length to weight  
  length_bss[r,'WEIGHT'] <- exp(coef_wl1$log_a + coef_wl1$b * log(l))
  
  if (sp %in% unique(x = data_sratio3$scientific_name) & 
      l %in% data_sratio3$LENGTH) {
    sr <- data_sratio3[which(data_sratio3$scientific_name == sp 
                             & data_sratio3$LENGTH == l), 'mean']
    sr90 <- data_sratio3[which(data_sratio3$scientific_name == sp 
                               & data_sratio3$LENGTH == l), 'p90']
    sr10 <- data_sratio3[which(data_sratio3$scientific_name == sp 
                               & data_sratio3$LENGTH == l), 'p10']
    length_bss[r, 'SR']  <- sr
    length_bss[r, 'SR90']  <- sr90
    length_bss[r, 'SR10']  <- sr10
  } else if (sp %in% unique(x = data_sratio3$scientific_name) 
             & (l+5) %in% data_sratio3$LENGTH) { #some species have different bins (X5 instead of X0)
    sr <- data_sratio3[which(data_sratio3$scientific_name == sp 
                             & data_sratio3$LENGTH == (l+5)), 'mean']
    sr90 <- data_sratio3[which(data_sratio3$scientific_name == sp 
                               & data_sratio3$LENGTH == (l+5)), 'p90']
    sr10 <- data_sratio3[which(data_sratio3$scientific_name == sp 
                               & data_sratio3$LENGTH == (l+5)), 'p10']
    length_bss[r,'SR']  <- sr
    length_bss[r,'SR90']  <- sr90
    length_bss[r,'SR10']  <- sr10
  } else if (sp %in% unique(data_sratio3$scientific_name) 
             & (l-5) %in% data_sratio3$LENGTH) { #some species have different bins (X5 instead of X0)
    sr <- data_sratio3[which(data_sratio3$scientific_name == sp 
                             & data_sratio3$LENGTH == (l-5)), 'mean']
    sr90 <- data_sratio3[which(data_sratio3$scientific_name == sp 
    ...
    length_bss[r, 'SR10'] <- sr10
             ...
             & l < min(data_sratio3$LENGTH)) { #if smaller than min, then get SR of min length
    sr <- data_sratio3[
      which(data_sratio3$scientific_name == sp 
            & data_sratio3$LENGTH == min(data_sratio3$LENGTH)), 
      'mean']
    sr90 <- data_sratio3[
      which(data_sratio3$scientific_name == sp 
            & data_sratio3$LENGTH == min(data_sratio3$LENGTH)), 
    ...
    length_bss[r,'SR10']  <- sr10
             ...
             & l > max(data_sratio3$LENGTH)) { #if bigger than min, then get SR of max length
    sr <- data_sratio3[
      which(data_sratio3$scientific_name == sp 
            & data_sratio3$LENGTH == max(data_sratio3$LENGTH)), 
      'mean'
    ]
    sr90 <- data_sratio3[
      which(data_sratio3$scientific_name == sp 
...
#check NAs in SR
length_bss[is.na(x = length_bss$SR),]
unique(x = length_bss[is.na(x = length_bss$SR),'scientific_name'])

#get subsample info
freq_subsamp <- aggregate(FREQUENCY ~ HAULJOIN + SPECIES_CODE, 
                          data = length_bss,
                          FUN=sum)
freq_subsamp1 <- merge(x = data_length$catch, 
                       y = freq_subsamp, 
...
#merge total haul
length_bss <- merge(x = length_bss,
                    y = freq_subsamp1[,c(1,2,4,5)],
                    by = c('HAULJOIN','SPECIES_CODE'))

#expanded frequency
length_bss$FREQ_EXP <- 
  length_bss$FREQUENCY * length_bss$NUMBER_FISH / length_bss$FREQ_SUBSAMP
...
length_bss_clean <- length_bss[is.finite(x = length_bss$SR), ]
...
meansr <- unique(x = length_bss_clean[, c("scientific_name", 'LENGTH', 'SR')])
aggregate(SR ~ scientific_name, data = meansr, FUN = mean)

#weight by species for each haul
wl <- aggregate(cbind(WEIGHT_FREQ, ADJ_WEIGHT_FREQ, 
                      ADJ_WEIGHT_FREQ90, ADJ_WEIGHT_FREQ10) ~ 
                  scientific_name + YEAR + HAULJOIN,
                data = length_bss,
                FUN = sum)

#total per year
wl1 <- aggregate(cbind(WEIGHT_FREQ, ADJ_WEIGHT_FREQ) ~ scientific_name + YEAR,
                 data = length_bss, 
                 FUN = sum)
...
wc <- aggregate(WEIGHT ~ scientific_name + YEAR + HAULJOIN,
                data = catch_bss,
                FUN=sum)
...
for (sp in spp_vect) {
  ...
  data_geostat <- readRDS(file = paste0('data/data_processed/species/', sp, 
                                        '/data_geostat.rds')) |>
    subset(subset = survey_name == 'Eastern Bering Sea Slope Bottom Trawl Survey')
  
  #weigth adjusted SR
  wl1 <- subset(x = wl, 
                subset = scientific_name == sp,
                select = c('scientific_name', 'HAULJOIN', 'ADJ_WEIGHT_FREQ',
                           'ADJ_WEIGHT_FREQ90', 'ADJ_WEIGHT_FREQ10'))
  ...
  data_geostat2 <- merge(x = data_geostat1, y = wl1,
                         ...
                         all.x = TRUE)
  data_geostat2$ADJ_WEIGHT_FREQ[is.na(x = data_geostat2$ADJ_WEIGHT_FREQ)] <- 0
  
  #convert grams to kg/ha
  data_geostat2$ADJ_KG_HA <- 
    data_geostat2$ADJ_WEIGHT_FREQ / data_geostat2$effort / 1000
  data_geostat2$ADJ_KG_HA90 <- 
    data_geostat2$ADJ_WEIGHT_FREQ90 / data_geostat2$effort / 1000
  data_geostat2$ADJ_KG_HA10 <- 
    data_geostat2$ADJ_WEIGHT_FREQ10 / data_geostat2$effort / 1000
  ...
  if (sp == 'Reinhardtius hippoglossoides') {
    data_geostat2 <- subset(x = data_geostat2, subset = cpue_kgha < 700)
  }
  
  #plot
  p <- 
    ggplot() +
    geom_point(data = subset(data_geostat2, cpue_kgha != 0), 
