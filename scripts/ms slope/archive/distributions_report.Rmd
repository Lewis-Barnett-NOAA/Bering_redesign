...
These lines of code set the working directory and load necessary libraries (installing them if needed).
...
This report describes the analysis of species distribution in the Eastern Bering Sea (EBS) shelf, EBS slope, and Northern Bering Sea (NBS). We used observed and simulated observations from VAST models to explore various metrics and test the hypothesis that species shift their distribution during warm years to occupy northern and deeper regions in the area.
...
Annual SBT values were extracted as the mean SBT value over cells from the ROMS Bering-10K model extrapolation grid. Annual mean SBT values were computed for multiple regions: EBS shelf, EBS slope, NBS and all (EBSshelf+EBSslope+NBS). We scaled annual SBT estimates to identify years warm and cold years as well as warm and cold periods. Identified cold and warm years were years with one standard deviation from the mean value. Cold and warm labeling was compared to previous temperature analysis in the area (Nielsen *et al*., 2024) (2002-2005 warm; 2006-2013 cold; 2014:2022 warm)
...
ggplot() +
  ...
  geom_point(data = subset(temp_region1, year %in% 1982:2022) ,
...
*Average scaled annual SBT across regions. Horizontal rectangular shade represents the 1 standard deviation that includes normal SBT estimates. Vertical rectangular shade represents the time period in which slope model was fitted. Bold x-axis text indicates years with slope observed data. Dashed blue lines represent cold years and dashed red lines represent warm years. Point colors represent regions.*

## Metrics

-   Note that this analysis doesn't account for catchability differences between the EBS+NBS shelf survey data and EBS slope survey data, which are being analyzed in Rohan *et al.* (in progress).
...
load('data/data_processed/obs_depth_density.RData')
...
### Empirical depth distribution of occurrence by temperature regime
...
### Observed CPUE by temperature regime and depth bins
...
ggplot(data=subset(obs_df1,species %in% sel_sp))+
    geom_boxplot(aes(x=year,y=log(1+cpue_kgkm2),
                 ...
                 outliers = FALSE,alpha=0.9)+ #outliers = FALSE,
...
### Fraction of total abundance by region
...
### Predicted CPUE by temperature regime and region
...
#load file observed dataframe input 
load('data/data_processed/dens_sel.RData') #dens_all
...
### Latitudinal and bathymetrical COG relative to temperature and biomass and effective area occupied
...
load('data/data_processed/dens_shelfslope.RData') #dens_all
...
#save table
save(metrics_df,file='data/data_processed/metrics_df.RData')
...
#plot effective area
ggplot(data=subset(metrics_df,species %in% sel_sp))+
    geom_rect(data = rect_data, aes(ymin = -Inf, ymax = Inf, 
...
### Range edges relative to cold pool area
...
```{r, echo=FALSE, warning=FALSE,message=FALSE, fig.width=20, fig.height=13}
...
#filter SBT years
temp_region2<-subset(temp_region1,year %in% c(2002:2016))
...
for (s in shelfslope_spp) {
  ...
  # Convert df to data.table for faster operations
  df <- as.data.table(df)
  ...
  # Precompute the raster extent and CRS
  base_extent <- extent(raster())
  base_crs <- CRS(paste0("+proj=aea +lat_1=55"," +lat_2=65 +lat_0=50 ",
  ...
  for (y in 2002:2016) {
    ...
    all_bio4 <- df[Year == y]
    all_bio4[, quantile := as.integer(dens < p2 & dens > p1)]
  
    # Convert to sf and rasterize
    all_bio5 <- st_as_sf(all_bio4, coords = c("Lon", "Lat"), 
                         crs = 4326) %>% st_transform(crs = st_crs(base_crs))
    ...
    #contour of cold pool
    r4b <- st_as_sf(rasterToPolygons(r2$Temp, dissolve = TRUE))
    
    # Temp region and year for label
    itemp_df <- temp_region2[year == y]
    temp_min <- min(temp_region2$temp)
    temp_max <- max(temp_region2$temp)
    itemp <- itemp_df$temp
    ...
    p <- ggplot() +
      ...
      coord_sf(crs = base_crs, 
               xlim = c(-1516559.21, -47636.05), ylim = c(383099.5, 1994909.7), 
               label_axes = "-NE-") +
  ...
  #all plots
  plots<-plot_grid(plotlist = frames,ncol = 5) 
    
  # Title with reduced font size
  title <- ggdraw() + 
               draw_label(s, fontface = 'bold', size = 20) + 
               theme(plot.margin = margin(0, 0, 0, 0))

  # Grid plot with adjusted relative heights
...
### Cumulative distribution of predicted biomass relative to depth
...
load('data/data_processed/metrics_df.RData') #metrics_df
...
### Bathymetrical COG of adults excluding slope data

Latitudinal COG relative to bathymetrical COG using adult data relative to SBT in the regions

```{r, fig.width=10, fig.height=9}

#plot without slope years
ggplot()+
...
Latitudinal COG relative to bathymetrical COG using adult data relative to SBT in the regions. Slope years were removed to avoid bias towards slope years for the bathymetrical COG.
